...
from .quote_adaptor_biniance import BinanceQuoteEngine
...
def run() -> None:
    ...
    engine = BinanceQuoteEngine(sandbox=args.sandbox, timeout_ms=args.timeout_ms, callback=PrintQuoteCallback())
    try:
        engine.connect()
        engine.process_quote(args.symbol, order_book_limit=args.order_book_limit, trade_limit=args.trade_limit)
    finally:
        engine.close()
